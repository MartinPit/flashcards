  ...
  const save = async () => {
    ...
    try {
      const duplicate = await powerSync.execute(
        "SELECT id FROM folders WHERE user_id = ? AND is_card_set = 1 AND name = ? AND COALESCE(parent_id, '') = ? LIMIT 1",
        [session.user.id, trimmed, parentId || '']
      );

      if (duplicate.rows.length > 0) {
        setError('A card set with that name already exists in this location.');
        return;
      }

      await powerSync.execute(
        "INSERT INTO folders (id, user_id, name, parent_id, created_at, is_card_set) VALUES (?, ?, ?, ?, datetime('now'), 1)",
        [randomUUID(), session.user.id, trimmed, parentId || null]
      );

      dismiss();
    } catch (e) {
      console.error('Failed to create card set', e);
      setError('Could not create card set.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Portal>
      <Dialog visible={visible} onDismiss={dismiss}>
        <Dialog.Title>New Card Set</Dialog.Title>
        <Dialog.Content>
          <Text variant="bodyMedium" style={{ marginBottom: 12, opacity: 0.7 }}>
            Card sets contain cards and live inside folders.
          </Text>
          <TextInput
            label="Card set name"
            value={name}
